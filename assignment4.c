    ...
    */
    if (frame != -1){
        ...
        if (page_replacement_scheme == REPLACE_FIFO) {
            ...
            //FIFO SHIT --> Lol figure out if I need to implement the whole thing? m
            //I think this gets called over and over in a loop so maybe a bunch of "if queue is full do this"
            //if queue is empty do this
            //if queue is not full or empty do this

            /*

            page_table contains all entries
            probably just pop page_table[0] out of there

            for int i = 1; i < page_table size ; i++
                page_table[i] = page_table i + 1; 

            //update global vars if necessary? maybe memory size or page size or sumshit
            //definitely look into swap ins/outs
